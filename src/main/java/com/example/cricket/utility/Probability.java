...
public class Probability {
    ...
    public BallOutcome run(String type) {
        ...
        if (randomValue < 5) {
            return new BallOutcome(BallType.WIDE, wideRuns[rand.nextInt(wideRuns.length)]);
        }
        if (randomValue < 10) {
            return new BallOutcome(BallType.NO_BALL, noBallRuns[rand.nextInt(noBallRuns.length)]);
        }

        int runScored = normalRuns[rand.nextInt(normalRuns.length)];
        if (runScored == 7) {
            return new BallOutcome(BallType.NORMAL, 7);
        }
